...
export async function getTopNewsArticles(params: NewsArticleParams): Promise<any> {
	...
	try {
		const response = await fetch('https://api.thenewsapi.com/v1/news/top?' + query, options);
		const result = await response.text();
		let jsonResult = JSON.parse(result);
		let attempts = 0;
		while (jsonResult.meta.returned === 0 && attempts < 5) {
			const retryResponse = await fetch('https://api.thenewsapi.com/v1/news/top?' + query, options);
